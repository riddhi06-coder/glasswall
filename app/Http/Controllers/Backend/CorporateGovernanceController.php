...
class CorporateGovernanceController extends Controller
{
    ...
    private const DIR = 'governance-docs';

    public function index()
    {
        $reports = GovernanceDocument::orderBy('priority')->orderBy('id')->get();

        return view('backend.investors.corporate_governance.index', compact('reports'));
    }

    public function create()
    {
        $isFirst = GovernanceDocument::count() === 0;

        return view('backend.investors.corporate_governance.create', compact('isFirst'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('manage-corporate-governance.index')->with('message', 'Governance document added successfully.');
    }

    public function edit($id)
    {
        $report  = GovernanceDocument::findOrFail($id);
        $isFirst = $report->id === GovernanceDocument::orderBy('id')->value('id');

        return view('backend.investors.corporate_governance.edit', compact('report', 'isFirst'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('manage-corporate-governance.index')->with('message', 'Governance document updated successfully.');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('manage-corporate-governance.index')->with('message', 'Governance document deleted successfully.');
    }
    ...
    private function rules(bool $isFirst, ?int $ignoreId = null): array
    {
        ...
        $rules = [
            ...
            'pdf'       => "{$req}|file|mimes:pdf|max:5120",
    ...
    private function messages(): array
    {
        return [
            ...
            'pdf.max'                 => 'The PDF may not be larger than 5 MB.',
